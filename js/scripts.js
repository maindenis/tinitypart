...
function setInfoTablePosition() {
    if($(".orders_table .imp_2.active").length > 0) {
        if(bodyWidth <= 1240) {
            topCoord = $(".orders_table .imp_2.active").offset().top + 10;
            leftCoord = $(".orders_table .imp_2.active").offset().left - ($(".info_table").width()/2) + 10;
        } else {
            topCoord = $(".orders_table .imp_2.active").offset().top;
            leftCoord = $(".orders_table .imp_2.active").offset().left + 20;
        }
        if($(".info_table").hasClass("active")) {
            $(".info_table").attr("style", "");
            $(".info_table").offset({top: topCoord, left: leftCoord});
        }
    }
}

function getSearchPositionParams() {
    if($(".header_search_sect").length > 0) {
        if(bodyWidth > 768) {
            searchWrapp = $(".header_search_sect").closest(".novisible_768");
            if($(document).scrollTop() >= searchWrapp.offset().top) {
                searchWrappHeight = $(".header_search_sect").height();
                searchWrapp.height(searchWrappHeight);
                $(".header_search_sect").addClass("fixed");
            } else {
                $(".header_search_sect").removeClass("fixed");
            }
        } else {
            $(".header_search_sect").removeClass("fixed");
            searchWrapp.css({"height" : "auto"});
        }
    }
}
...
$(document).ready(function() {
   ...
   $(document).on("click", "[data-popup-link]",  function(e) {
    ...
    $(".main_checkbox input").on("change", function() {
      parentBlock = $(this).closest(".checkboxes_array");
      chChildrens = parentBlock.find(".ch_childrens input");
      chMains = parentBlock.find(".main_checkbox input");
      if (!$(this).is(":checked")) {
        chChildrens.prop("checked", false);
        chMains.prop("checked", false);
      } else {
        chChildrens.prop("checked", true);
        chMains.prop("checked", true);
      }
    });
    ...
    $(".filter_form_resp > input").on("keyup", function() {
        var input, filter, ul, li, a, i, txtValue;
        input = this;
        if(input.hasAttribute("data-table-index")) {
            index = $(this).attr("data-table-index");
            parentTable = $("[data-table-parent-index = '"+index+"']")
        } else {
            parentTable = $(this).closest(".sortTable");
        }        
    ...
    $(".orders_table .imp_2").on("click", function(e) {
        e.preventDefault();
        $(".info_table").removeClass("active");
        $(".orders_table .imp_2").removeClass("active");
        $(this).addClass("active");
        if(bodyWidth <= 1240) {
            topCoord = $(this).offset().top + 30;
            leftCoord = $(this).offset().left - $(".info_table").width()/2 + 20;
        } else {
            topCoord = $(this).offset().top;
            leftCoord = $(this).offset().left + 20;
        }
        if(!$(".info_table").hasClass("active")) {
            $(".info_table").addClass("active");
            $(".info_table").offset({top: topCoord, left: leftCoord});
        }
    });
